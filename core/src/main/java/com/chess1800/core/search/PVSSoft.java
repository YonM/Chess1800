...
public class PVSSoft extends PVS {
    ...
    protected int PVS(int ply, int depth, int alpha, int beta) throws SearchRunException {
        ...
        for (int i = 0; i < num_moves; i++) {
            ...
            if (board.makeMove(moves[i])) {
                pvIndex = i;
                movesFound++;
                bestScore = -PVS(ply + 1, depth - 1, -beta, -alpha);
                board.unmakeMove();
                if (bestScore > alpha) {
        ...
        for (int i = pvIndex; i < num_moves; i++) {
            ...
            if (board.makeMove(moves[i])) {
                ...
                if (movesFound > LATEMOVE_THRESHOLD && depth > LATEMOVE_DEPTH_THRESHOLD && !board.isCheck() && !Move.isCapture(moves[i])) {
                    ...
                    score = -PVS(ply + 1, depth - 1, -alpha - 1, -alpha); // PVS Search
                }
                if ((score > alpha) && (score < beta)) {
                    score = -PVS(ply + 1, depth - 1, -beta, -alpha); //Better move found, re-search with Open Window
                    if (score > alpha)
                        alpha = score;
                }
                board.unmakeMove();
    ...
    protected int quiescenceSearch(int ply, int alpha, int beta) throws SearchRunException {
        ...
        if (board.isCheck()) return PVS(ply, 1, alpha, beta);
