...
//        legalMoves = 0;
        ...
        for (int currentDepth = 1; currentDepth <= MAX_DEPTH; currentDepth++) {
            ...
            null_allowed = true;
            if(failHard)
                score = alphaBetaPVSHard(b, 0, currentDepth, alpha, beta);
            else
                score = alphaBetaPVSSoft(b, 0, currentDepth, alpha, beta);
    ...
    */
    private int alphaBetaPVSHard(Board b, int ply, int depth, int alpha, int beta) {
        evals++;
        ...
        if (depth <= 0) {
            follow_pv = false;
            return quiescenceSearchHard(b, ply, alpha, beta);
        }
        ...
        if (!follow_pv && null_allowed) {
            if (b.movingSidePieceMaterial() > NULLMOVE_THRESHOLD) {
                if (!b.isCheck()) {
                    null_allowed = false;
                    b.makeNullMove();
                    score = -alphaBetaPVSHard(b, ply, depth - NULLMOVE_REDUCTION, -beta, -beta + 1);
                    b.unmakeMove();
                    null_allowed = true;
                    if (score >= beta) {
        ...
        for (int i = 0;i<num_moves;i++) {
            if (b.makeMove(moves[i])) {
                pvIndex=i;
                movesFound++;
                score = -alphaBetaPVSHard(b, ply + 1, depth - 1, -beta, -alpha);
                b.unmakeMove();
                if (score > alpha) {
        ...
        for (int i = pvIndex; i < num_moves; i++) {
            ...
            if (b.makeMove(moves[i])) {
                ...
                if (movesFound > LATEMOVE_THRESHOLD && depth > LATEMOVE_DEPTH_THRESHOLD && !b.isCheck() && !MoveAC.isCapture(moves[i])) {
                    score = -alphaBetaPVSHard(b, ply + 1, depth - 2, -alpha - 1, -alpha);
                } else {
                    score = -alphaBetaPVSHard(b, ply + 1, depth - 1, -alpha - 1, -alpha); // PVS Search
                }
                if ((score > alpha) && (score < beta)) {
                    score = -alphaBetaPVSHard(b, ply + 1, depth - 1, -beta, -alpha); //Better move found, normal alpha-beta.
                }
    ...
    private int alphaBetaPVSSoft(Board b, int ply, int depth, int alpha, int beta) {
        evals++;
        triangularLength[ply] = ply;
        // Check if time is up
        if(!useFixedDepth) {
        ...
        if (depth <= 0) {
            follow_pv = false;
            return quiescenceSearchSoft(b, ply, alpha, beta);
        }
        ...
        if (!follow_pv && null_allowed) {
            if (b.movingSidePieceMaterial() > NULLMOVE_THRESHOLD) {
                if (!b.isCheck()) {
                    null_allowed = false;
                    b.makeNullMove();
                    score = -alphaBetaPVSSoft(b, ply, depth - NULLMOVE_REDUCTION, -beta, -beta + 1);
                    b.unmakeMove();
                    null_allowed = true;
                    if (score >= beta) {
        ...
        for (int i = 0;i<num_moves;i++) {
            if (b.makeMove(moves[i])) {
                pvIndex=i;
                movesFound++;
                bestScore = -alphaBetaPVSSoft(b, ply + 1, depth - 1, -beta, -alpha);
                b.unmakeMove();
                if (bestScore > alpha) {
        ...
        for (int i = pvIndex; i < num_moves; i++) {
            ...
            if (b.makeMove(moves[i])) {
                ...
                //Late Move Reduction
                if (movesFound > LATEMOVE_THRESHOLD && depth > LATEMOVE_DEPTH_THRESHOLD && !b.isCheck() && !MoveAC.isCapture(moves[i])) {
                    score = -alphaBetaPVSSoft(b, ply + 1, depth - 2, -alpha - 1, -alpha);
                } else {
                    score = -alphaBetaPVSSoft(b, ply + 1, depth - 1, -alpha - 1, -alpha); // PVS Search
                }
                if ((score > alpha) && (score < beta)) {
                    score = -alphaBetaPVSSoft(b, ply + 1, depth - 1, -beta, -alpha); //Better move found, re-search with Open Window
                        if (score > alpha)
                            alpha = score;
                }
                b.unmakeMove();
    ...
    private int quiescenceSearchHard(Board b, int ply, int alpha, int beta) {
        triangularLength[ply] = ply;

        //Check if we are in check.
        if (b.isCheck()) return alphaBetaPVSHard(b, ply, 1, alpha, beta);
        ...
        for (int i = 0; i < num_captures; i++) {
            ...
            score = -quiescenceSearchHard(b, ply + 1, -beta, -alpha);
    ...
    private int quiescenceSearchSoft(Board b, int ply, int alpha, int beta) {
        triangularLength[ply] = ply;

        //Check if we are in check.
        if (b.isCheck()) return alphaBetaPVSSoft(b, ply, 1, alpha, beta);
        ...
        for (int i = 0; i < num_captures; i++) {
            b.makeMove(captures[i]);
            score = -quiescenceSearchSoft(b, ply + 1, -beta, -alpha);
            b.unmakeMove(captures[i]);
            if (score > alpha) {
                if (score >= beta) {
                    //Fail Soft
